...
export async function GET() {
  ...
  // 3. Fetch Users
    ...
    const { data: heartRateDataRaw, error: heartRateError } = await supabase.rpc('get_heart_rate_data_all');
    ...
    // Note: Ensure entry.timestamp matches ISO format from DB
    ...
    const finalLeaderboard = users.map(user => {
      // FIX: Look up directly by display_name (O(1)) instead of .find (O(N))
      ...
      return {
        id: user.username, // Changed from user.id to user.username
